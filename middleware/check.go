...
func permissionCheckHelper(c *gin.Context, requiredPermission int) {
	...
	if role == nil || role.(int) < requiredPermission {
		if c.Request.URL.Path == "/explorer" || c.Request.URL.Path == "/video" {
...
func VideoDownloadPermissionCheck() func(c *gin.Context) {
	return func(c *gin.Context) {
		permissionCheckHelper(c, common.VideoDownloadPermission)
	}
}
